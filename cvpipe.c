/* cvpipe.c — Merged CVPipe: Con-verse Prime Pipeline (single-pass, no disk 
 *            I/O)
 *
 ...
 */
static int comput_zone(srch_zone_t *zones, mpz_t prime_min, mpz_t prime_max) {
    ...
    for (int d = d_min; d <= d_max; d++) {
        ...
        for (int i = 0; i < NUM_PATTERNS && zone_count < MAX_ZONES; i++) {
            ...
            mpz_set(p_min, mpz_cmp(p_zone_min, prime_min) < 0 ? prime_min : 
                    p_zone_min);
            mpz_set(p_max, mpz_cmp(p_zone_max, prime_max) > 0 ? prime_max : 
                    p_zone_max);
 ...
 */
static bool solve_consec_sqrs(mpz_t p, mpz_t n_out, mpz_t disc) {
    /* disc = 2p - 1 */
    mpz_mul_ui(disc, p, 2);
    mpz_sub_ui(disc, disc, 1);
...
int main(int argc, char *argv[]) {
    ...
    srch_zone_t zones[MAX_ZONES];
    int num_zones = comput_zone(zones, start_prime_z, max_prime_z);
                                     ...
                                     g_emirps,g_converse,g_otto)
    {
        ...
        for (int z = 0; z < num_zones; z++) {
            ...
            for (uint64_t n = first_n; n <= zone_end; n += NUM_THREADS) {
                ...
                mpz_mul_ui(candidate, candidate, 2);    // 2n^2 
                ...
                if (is_palindrome_str(cand_str, len)) {
                    ...
                    if (mpz_probab_prime_p(candidate, MR_ROUNDS) > 0) {
                        ...
                        if (solve_consec_sqrs(candidate, n1_out,\
                            disc)) {
                ...
                bool p_ok = solve_consec_sqrs(candidate, n1_out, disc);
                bool r_ok = solve_consec_sqrs(reversed_num, n2_out, disc);
                ...
                if (tid == 0 && st.candidates_generated % PROGRESS_INTERVAL 
                    == 0) {

                    double elapsed = omp_get_wtime() - omp_start;
                    
                    printf("  [%6.1fs] zone=%d/%d  n=%lu  gate=%lu  consec_sq=\
                            %lu  mr=%lu\n", elapsed, z + 1, num_zones, n,
                           st.gatekeeper_passed, st.consec_sq_passed,\
                           st.mr_tests);
